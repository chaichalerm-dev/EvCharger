...
MapLibre GL JS renders OpenStreetMap raster tiles without a paid token. A local diagrammatic road/context layer remains visible when the external tile service cannot be reached, so the primary location-selection flow does not become a blank screen. Opportunity and entity sources are GeoJSON; opportunities use client clustering. Entity categories include EV stations, competitors, gas stations, POIs, and partner branches.
