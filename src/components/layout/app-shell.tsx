...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
    <aside className={`sidebar ${menuOpen ? "mobile-open" : ""}`}>
      <div className="brand"><div className="brand-mark"><Zap size={21} /></div><div className="brand-copy"><div className="brand-title">EV Atlas</div><div className="brand-sub">Thailand Intelligence</div></div><button className="icon-btn sidebar-mobile-close" onClick={() => setMenuOpen(false)} aria-label={t("closeNavigation")}><X /></button></div>
    ...
    <div className="main-column">
      <header className="topbar">
        ...
        <button className="icon-btn mobile-menu" disabled={!interactive} onClick={() => setMenuOpen(value => !value)} aria-expanded={menuOpen} aria-label={t(menuOpen ? "closeNavigation" : "openNavigation")}><Menu /></button>
